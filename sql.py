...
def criarColecao(nome):
    ...
    with Session(engine) as sessao, sessao.begin():
        ...
        sessao.execute(
            text("INSERT INTO colecao (nome) VALUES (:nome)"), report)
...
def obterColecao(nome):
    with Session(engine) as sessao:
        ...
        colecao = sessao.execute(
            text("SELECT id, nome FROM colecao WHERE nome = :nome"), parametros).first()
...
def listarColecoes():
    ...
    with Session(engine) as sessao:
        colecoes = sessao.execute(
            text("SELECT id, nome FROM colecao ORDER BY id"))
...
def criarRelatorio(relatorio):
    ...
    report = {
        ...
        'preco': float(relatorio[4].replace(',', '').replace('K', '000').replace(' ', '').replace(';', '').replace('&lt', '').replace('USDC', '').replace('ETH', '').replace('MATIC', '').replace('W', '').replace('BNB', '').replace('—', '0')),
    ...
    with Session(engine) as sessao, sessao.begin():
        sessao.execute(text(
            f"INSERT INTO relatorioDiario (colecao_id, posicao, colecao_nome, volume, variacao, preco, vendas, donosUnicos, listagem, dia) VALUES ({validaExistencia[0]}, :posicao, :colecao_nome, :volume, :variacao, :preco, :vendas, :donosUnicos, :listagem, :data)"), report)
...
def listarRelatoriosDiarios():
    ...
    with Session(engine) as sessao:
        reports = sessao.execute(text(
            "SELECT * FROM relatorioDiario INNER JOIN colecao ON relatorioDiario.colecao_id = colecao.id ORDER BY dia DESC"))
